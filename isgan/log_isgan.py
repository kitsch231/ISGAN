...
from en_de import decrypt,encrypt
...
class ISGAN(object):
    ...
    def train(self, epochs, batch_size=4):
        ...
        for k in range(images_rgb.shape[0]):
            ...
            # a=rgb2gray(imgs_rgb[k, :, :, :])[0, :, :]
            # plt.imshow(a)
            # plt.show()
            secret_en=encrypt(rgb2gray(images_rgb[k, :, :, :])[0,:,:],key)#加密操作
            secret_gray[k, 0, :, :] =secret_en
        print('加密已完成')
    ...
    def draw_images(self, nb_images=1, epoch=1):
        ...
        secret_write = np.zeros((imgs_secret.shape[0], 1, imgs_cover.shape[2], imgs_cover.shape[3]))

        key = [0.343, 0.432, 0.63, 3.769, 3.82, 3.85, 1]
        # 载体图和隐写图转换为灰度图
        for k in range(nb_images):
            images_ycc[k, :, :, :] = rgb2ycc(imgs_cover[k, :, :, :])
            # a=rgb2gray(imgs_secret[k, :, :, :])[0, :, :]
            # print(a.shape)
            # plt.imshow(a)
            # plt.show()
            secret_en = encrypt(rgb2gray(imgs_secret[k, :, :, :])[0, :, :], key)#加密操作
            secret_gray[k, 0, :, :] = secret_en
            secret_write[k, 0, :, :] = rgb2gray(imgs_secret[k, :, :, :])
        ...
        secret_write = np.reshape(secret_write, (nb_images, 256, 256))


        for k in range(nb_images):
            imageio.imwrite('images/{}_{}_cover.png'.format(epoch, k), imgs_cover[k, :, :, :])
            imageio.imwrite('images/{}_{}_secret.png'.format(epoch, k), secret_write[k, :, :])
            imageio.imwrite('images/{}_{}_stego.png'.format(epoch, k), imgs_stego[k, :, :, :])

            # print(imgs_recstr[k, :, :][:,:].shape)
            # plt.imshow(imgs_recstr[k, :, :][:,:])
            # plt.show()
            imgs_recstr_de=decrypt(imgs_recstr[k, :, :][:,:],key)#解密操作
            # plt.imshow(imgs_recstr_de)
            # plt.show()

            imageio.imwrite('images/{}_{}_recstr.png'.format(epoch, k),imgs_recstr_de)
